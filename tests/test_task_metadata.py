...
def test_task_metadata_records_family_agnostic_constraints() -> None:
    ...
    assert metadata == {
        "schema_version": "agades.pqc.task_metadata.v6",
        ...
        "operator_params": [{"beta": 48, "svp_cost_model": "ADPS16"}],
    ...
    assert TASK_METADATA_SCHEMA == "agades.pqc.task_metadata.v6"
...
def test_task_metadata_match_allows_id_variant_but_rejects_wrong_task() -> None:
    ...
    wrong_param_data = dict(lattice_data)
    wrong_param_data["operators"] = [
        {**operator, "params": {**operator["params"], "beta": 49}}
        for operator in wrong_param_data["operators"]
    ]
    wrong_param_plan = AttackPlan.model_validate(wrong_param_data)
    ...
    assert not attack_plan_matches_task_metadata(wrong_param_plan, metadata)
    assert attack_plan_matches_task_metadata(
        wrong_param_plan,
        metadata,
        allow_operator_param_variants=True,
    )
