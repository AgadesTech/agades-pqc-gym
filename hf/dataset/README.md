...
- `task_metadata.jsonl` with `agades.pqc.task_metadata.v4` rows for valid public AttackPlans, recording target family, target name, support level, ordered operator types, reproducibility requirement, public flag, source path, seed AttackPlan SHA-256 digest, seed verifier status/reward, seed estimator, and seed reproduction status so Prime-style and Hugging Face consumers can keep candidate mutations on the intended task without requiring an exact `attack_plan_id` copy.
