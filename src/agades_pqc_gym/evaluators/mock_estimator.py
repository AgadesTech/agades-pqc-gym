...
class MockEstimatorAdapter:
    ...
    def estimate(self, plan: AttackPlan) -> EstimatorResult:
        ...
        target_n = plan.target.n or 1
        beta = _max_int_param(plan, "beta", default=max(32, target_n // 2))
        operator_adjustment = _operator_adjustment(plan)
        q_bits = math.log2(plan.target.q or 2)
        module_factor = plan.target.k if plan.target.k is not None else 1
        dimension = target_n * module_factor
...
def _operator_adjustment(plan: AttackPlan) -> float:
    ...
    for operator in plan.operators:
        if operator.type == "modulus_switching":
            ...
            target_q = plan.target.q or 2
            if isinstance(q_prime, int) and q_prime < target_q:
                adjustment += min(8.0, math.log2(target_q / q_prime) * 2.0)
