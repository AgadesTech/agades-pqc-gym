...
Prime task rows provide `agades.pqc.task_metadata.v6` `info` metadata with
`target_family`, `target_name`, `support_level`, ordered `operator_types`,
ordered `operator_params`, ordered `operator_assumptions`, the seed AttackPlan SHA-256 digest, seed
...
AttackPlans, repair candidates that change attack parameters, or candidates
that drop required hypotheses; changing `attack_plan_id` alone is allowed so
candidates can be treated as variants instead of exact copies. Semantic-mutation
tasks may change operator params only when the challenge explicitly requires a
semantic variant.
